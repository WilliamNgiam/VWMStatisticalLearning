...
% * Examines awareness of the participant in the objective test
%
% This code runs the analysis for Experiment 1a
%
% ----------------------------------------------------------------------- %

wantIndvPerf = 0;               % Produces plots of each individual's performance
wantCombinedIndvPerf = 0;       % Combines plots of each individual's performance into one figure
wantAggregatePerf = 1;          % Plots aggregate performance
wantProbPairs = 0;              % Calculates the number of low-probability pairs shown and number of trials containing only high-probability pairs
wantTestAwareness = 0;          % Examines each participant's awareness of colour pairings
...
%     disp(nCorr);
    ...
    if block.thisCondition == 1  % Uniform first
        ...
        for thisBlock = 1:experiment.nBlocks
            ...
            if thisBlock <= experiment.nBlocks/2

                thisConditionBlock = thisBlock;
            ...
            elseif thisBlock > experiment.nBlocks/2

                thisConditionBlock = thisBlock-(experiment.nBlocks/2);
...
if wantAggregatePerf

    % Figure of mean data

    figure('Name','Aggregate of All Participants');
    Conditions = {'Uniform', 'Pattern'};
    subplot(2,1,1);
...
if wantTestAwareness
    
    % Testing awareness

    cd('C:\Users\Dirk VU\Documents\MATLAB\Will\VWMStatisticalLearning\UserData');
    ...
    for thisParticipantNum = 1:numParticipants
        ...
        if whichCondnFirst == 1 % Uniform first
            ...
            for thisColour = 1:stimulus.nColours
                ...
                theirResponse = awareness.secondResponseColours(thisColour);            % Retrieve their response to what was paired to this Colour
        ...
        elseif whichCondnFirst == 2 % Patterned first

            for thisColour = 1:stimulus.nColours

                theirResponse = awareness.firstResponseColours(thisColour);
                thisPosition = find(stimulus.highProbPairs_Code == thisColour); % Retrieve the position of the colour in the 4x2 matrix
